...
class PreferenceSave {
    ...
    private val TOP_KEY = "com.samueljbeck.top.key"
    ...
    fun setBool(context: Context, key: String, value: Boolean?) {
        set(context, TOP_KEY + key, if (value != null && value) {
            "true"
        } else {
            "false"
        })
    ...
    fun getBool(context: Context, key: String): Boolean {
        return get(context, TOP_KEY + key).equals("true", ignoreCase = true)
    }
}